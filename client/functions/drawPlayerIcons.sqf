...
if (isNil "missionEH_drawPlayerIcons") then
{
	...
	logicPlayerIcon = compileFinal str (call currMissionDir + "client\icons\igui_side_indep_ca.paa");
}
else
{
...
missionEH_drawPlayerIcons = addMissionEventHandler ["Draw3D",
{
	if (!visibleMap && {showPlayerIcons}) then
	{
		...
		{
			if (alive _x && {side _x == playerSide || side _x == sideLogic} && {_x != player}) then
			{
