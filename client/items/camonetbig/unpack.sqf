...
if (MF_ITEMS_CAMO_NET_BIG call mf_inventory_count <= 0) exitWith {
...
_hasFailed = {
	...
	switch (true) do {
		...
		case (MF_ITEMS_CAMO_NET_BIG call mf_inventory_count <= 0): {_text = ERR_NO_NETS};
...
if (_success) then {
	...
	_netting = MF_ITEMS_CAMO_NET_BIG_TYPE createVehicle _pos;
