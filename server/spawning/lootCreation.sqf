...
_odd1 = 15;					//The odds that a building is selected to place loot.
_odd2 = 35;					//The odds that the selected building's spots will have loot(almost like odds per room).
_odditem = 100;					//Chance of item instead of weapon
_oddweapitem =0;				//instead of the weapon picked in above declared chance above there's also a chance to spawn other items instead of a rifle like medkits,toolkits,optics,silencers etc.
...
_interval = 7200;				//Time (in sec.) to pass before a city spawns new loot (5400 - 1,5 hr)
